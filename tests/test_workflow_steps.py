...
def test_download_step_output_urls_handles_multiple_files(monkeypatch, tmp_path):
    ...
    assert ctx.get_output("fetch", "output_urls") == [
        "https://real.example.org/a.ttl",
        "https://real.example.org/b.ttl",
    ]
...
def test_download_step_passes_graph_mode(monkeypatch, tmp_path):
    captured = {}
    ...
    def fake_download(**kwargs):
        ...
        with open(os.path.join(local_dir, "a.ttl"), "w") as f:
            f.write("data")
        with open(os.path.join(local_dir, "a.ttl.graph"), "w") as f:
            f.write(kwargs["databusURIs"][0])
        kwargs["manifest_context"].record_file(
            url=kwargs["databusURIs"][0], status="success"
        )
    ...
    assert ctx.get_output("fetch", "output_files") == [str(tmp_path / "a.ttl")]
...
def test_deploy_step_webdav_mode_requires_all_three_fields():
    ...
    with pytest.raises(
        StepValidationError,
        match="requires 'webdav_url', 'remote', and 'path' together",
    ):
        step.run(
            {
                "name": "publish",
...
def test_deploy_step_webdav_mode_uploads_then_deploys(monkeypatch):
    ...
    def fake_upload(distributions, remote, path, webdav_url):
        ...
        return [
            {
                "url": "https://cloud.example.com/webdav/data/a.ttl",
                "checksum": "abc123",
                "size": 100,
            }
        ]

    def fake_deploy_from_metadata(
    ...
    monkeypatch.setattr(
        "databusclient.workflow.steps.webdav.upload_to_webdav", fake_upload
    )
    monkeypatch.setattr(
        "databusclient.workflow.steps.deploy_from_metadata", fake_deploy_from_metadata
    )
    ...
    assert captured["upload_args"][1:] == (
        "nextcloud",
        "datasets/mydata",
        "https://cloud.example.com/webdav",
    )
    assert captured["api_key"] == "key123"
    assert ctx.get_output("publish", "output_files") == [
        "https://cloud.example.com/webdav/data/a.ttl"
    ]
...
def test_deploy_step_classic_mode_still_works_with_urls(monkeypatch):
    ...
    monkeypatch.setattr(
        "databusclient.workflow.steps.create_dataset", fake_create_dataset
    )
    monkeypatch.setattr("databusclient.workflow.steps.api_deploy_call", fake_deploy)
